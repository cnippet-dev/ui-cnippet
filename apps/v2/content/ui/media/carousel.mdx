---
title: Carousel
description: A carousel with motion and swipe built using Embla. Built with Tailwind CSS. Copy-paste ready.
links:
  doc: https://www.embla-carousel.com/api/
---

<ComponentPreview name="v-carousel-1" />

## Installation

<CodeTabs>

<TabsList defaultValue="cli">
  <TabsTab value="cli">CLI</TabsTab>
  <TabsTab value="manual">Manual</TabsTab>
</TabsList>

<TabsPanel value="cli">

```bash
npx shadcn@latest add @cnippet/carousel
```

</TabsPanel>

<TabsPanel value="manual">

<Steps>

<Step>Install the following dependencies:</Step>

```bash
npm install embla-carousel-react
```

<Step>Copy and paste the following code into your project.</Step>

<ComponentSource name="carousel" title="components/ui/carousel.tsx" />

<Step>Update the import paths to match your project setup.</Step>

</Steps>

</TabsPanel>

</CodeTabs>

## Usage

```tsx
import {
  Carousel,
  CarouselContent,
  CarouselItem,
  CarouselNext,
  CarouselPrevious,
} from "@/components/ui/carousel"
```

```tsx
<Carousel className="mx-12">
  <CarouselContent>
    <CarouselItem>Slide 1</CarouselItem>
    <CarouselItem>Slide 2</CarouselItem>
    <CarouselItem>Slide 3</CarouselItem>
  </CarouselContent>
  <CarouselPrevious />
  <CarouselNext />
</Carousel>
```

## Anatomy

```tsx
<Carousel opts={{ loop: true }}> {/* <div role="region" aria-roledescription="carousel"> · data-slot="carousel" */}
  <CarouselContent> {/* scroll track · data-slot="carousel-content" */}
    <CarouselItem>…</CarouselItem> {/* one slide, repeat 1..n · data-slot="carousel-item" */}
  </CarouselContent>
  <CarouselPrevious /> {/* data-slot="carousel-previous" */}
  <CarouselNext /> {/* data-slot="carousel-next" */}
</Carousel>
```

## Good to know

- Built on Embla Carousel. Pass Embla options through `opts` and plugins through `plugins`. Plugins such as autoplay are separate packages (`embla-carousel-autoplay`) that you install yourself.
- `CarouselPrevious` and `CarouselNext` sit outside the carousel: 3rem to the left and right, or above and below when vertical. Leave room for them, e.g. `className="mx-12"` on `Carousel`, or reposition them with `className`.
- Each slide is full width by default. Show several at once with `basis-*` classes on `CarouselItem`, e.g. `basis-1/3`.
- To change the gap, change both sides: `-ml-4` on `CarouselContent` and `pl-4` on each `CarouselItem` (use `-mt-*` and `pt-*` when vertical).
- For programmatic control (current slide, dots, autoplay state), get the Embla API with `setApi`.
- The left and right arrow keys move between slides, even when `orientation="vertical"`.

## API Reference

### Carousel

Root carousel container powered by [Embla Carousel](https://www.embla-carousel.com/).

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `orientation` | `"horizontal" \| "vertical"` | `"horizontal"` | Scroll axis |
| `opts` | `EmblaOptionsType` | — | Embla options object (loop, align, dragFree, etc.) |
| `plugins` | `EmblaPluginType[]` | — | Embla plugins (autoplay, autoScroll, etc.) |
| `setApi` | `(api: CarouselApi) => void` | — | Callback to receive the Embla API instance for programmatic control |

### CarouselContent

Scroll container holding all `CarouselItem` children.

### CarouselItem

Individual slide. Use Tailwind `basis-*` utilities to control how many slides are visible at once.

### CarouselPrevious

Previous-slide button.

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `variant` | `ButtonVariant` | `"outline"` | Button visual style |
| `size` | `ButtonSize` | `"icon"` | Button size |

### CarouselNext

Next-slide button. Accepts the same `variant` and `size` props as `CarouselPrevious`.

## Examples

### Multiple Items

Shows multiple slides at once using fractional `basis` — each item takes half or a third of the track width, letting users see what's coming next.

<ComponentPreview name="v-carousel-2" />

### Custom Spacing

Reduces the gap between slides by overriding the default padding, useful for tighter layouts where breathing room should be minimal.

<ComponentPreview name="v-carousel-3" />

### Vertical Orientation

Switches the scroll axis to vertical via `orientation="vertical"` — suited for feeds, timelines, or any layout that naturally flows top-to-bottom.

<ComponentPreview name="v-carousel-4" />

### Autoplay with Indicators

Auto-advances every 2.5 s using the Embla API directly via `setInterval`, with pill-shaped dot indicators below that expand to show the active slide and allow manual jump.

<ComponentPreview name="v-carousel-5" />

### Testimonials

Single-slide carousel for customer quotes — each card shows a star rating, a pull quote, and an avatar with name and role; loops infinitely with arrow controls.

<ComponentPreview name="v-carousel-6" />

### Product Showcase

Multi-item carousel showing product cards with an image area, name, price, badge, and an Add to Cart button — aligned to `start` so partial next-slide is always visible.

<ComponentPreview name="v-carousel-7" />

### Onboarding Steps

Step-by-step wizard built on raw Embla with drag disabled — a progress bar and "Step N of M" label update as the user moves through each step using labeled Back / Next buttons.

<ComponentPreview name="v-carousel-8" />

### Team Members

Peek-style multi-item carousel for team cards — each card shows an avatar, name, role, department badge, and links; the partial next card signals more to explore.

<ComponentPreview name="v-carousel-9" />

### Gallery with Thumbnails

Main carousel controlled programmatically via `setApi` — clicking a thumbnail scrolls the main view to that image, and the thumbnail strip stays in sync as the user swipes.

<ComponentPreview name="v-carousel-10" />

### Testimonials

A looping testimonial carousel with a quote icon, reviewer name, role, and avatar initials. Dot indicators sync to the current slide and act as navigation buttons.

<ComponentPreview name="v-carousel-11" />

### Pricing Plans

A single-item carousel through Free, Pro, and Enterprise plans with arrow navigation. The Pro plan card is visually highlighted with a primary ring.

<ComponentPreview name="v-carousel-12" />

### Media Gallery

A carousel of labeled gradient slides with a slide counter and dot progress bar below, using the built-in `CarouselPrevious` / `CarouselNext` controls.

<ComponentPreview name="v-carousel-13" />

### Changelog

A drag-free multi-card carousel scrolling through release notes — each card shows a version tag, date, and a bullet list of changes.

<ComponentPreview name="v-carousel-14" />

### Feature Highlights

A slide-by-slide feature tour with an emoji icon, title, and description per slide. Dot indicators and Prev / Next buttons control navigation, with the last slide showing a "Done" checkmark.

<ComponentPreview name="v-carousel-15" />
