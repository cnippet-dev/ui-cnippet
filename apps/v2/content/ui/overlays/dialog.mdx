---
title: Dialog
description: A popup that opens on top of the entire page. Built with Base UI and Tailwind CSS. Copy-paste ready.

links:
  doc: https://base-ui.com/react/components/dialog#api-reference
---

<ComponentPreview name="v-dialog-1" />

## Installation

<CodeTabs>

<TabsList defaultValue="cli">
  <TabsTab value="cli">CLI</TabsTab>
  <TabsTab value="manual">Manual</TabsTab>
</TabsList>
<TabsPanel value="cli">

```bash
npx shadcn@latest add @cnippet/dialog
```

</TabsPanel>

<TabsPanel value="manual">

<Steps>

<Step>Install the following dependencies:</Step>

```bash
npm install @base-ui/react
```

<Step>Copy and paste the following code into your project.</Step>

<ComponentSource name="dialog" title="components/ui/dialog.tsx" />

<Step>Update the import paths to match your project setup.</Step>

</Steps>

</TabsPanel>

</CodeTabs>

## Usage

```tsx
import {
  Dialog,
  DialogClose,
  DialogDescription,
  DialogPanel,
  DialogFooter,
  DialogHeader,
  DialogPopup,
  DialogTitle,
  DialogTrigger,
} from "@/components/ui/dialog"
```

```tsx
<Dialog>
  <DialogTrigger>Open Dialog</DialogTrigger>
  <DialogPopup>
    <DialogHeader>
      <DialogTitle>Dialog Title</DialogTitle>
      <DialogDescription>Dialog Description</DialogDescription>
    </DialogHeader>
    <DialogPanel>Content</DialogPanel>
    <DialogFooter>
      <DialogClose>Close</DialogClose>
    </DialogFooter>
  </DialogPopup>
</Dialog>
```

**DialogFooter Variant**

The `DialogFooter` component supports a `variant` prop to control its styling:

- `default` (default): Includes a border-top, background color, and padding
- `bare`: Removes the border and background for a minimal appearance

```tsx
// Default variant (with border and background)
<DialogFooter>
  <DialogClose>Cancel</DialogClose>
  <Button>Save</Button>
</DialogFooter>

// Bare variant (no border or background)
<DialogFooter variant="bare">
  <DialogClose>Cancel</DialogClose>
  <Button>Save</Button>
</DialogFooter>
```

**DialogPanel Scrolling**

The `DialogPanel` component automatically wraps its content in a `ScrollArea` component. This means that if the content exceeds the dialog's maximum height, it will become scrollable automatically. The scrollbar will appear when needed, providing a smooth scrolling experience.

```tsx
<DialogPanel>
  {/* Long content that will scroll if it exceeds the dialog height */}
  <div>...</div>
</DialogPanel>
```

## Anatomy

```tsx
<Dialog> {/* root, owns the open state */}
  <DialogTrigger render={<Button variant="outline" />}>Edit profile</DialogTrigger> {/* <button> · data-slot="dialog-trigger" */}
  <DialogPopup> {/* portal + backdrop + popup + close (X) button · data-slot="dialog-popup" */}
    <DialogHeader> {/* data-slot="dialog-header" */}
      <DialogTitle>Edit profile</DialogTitle> {/* data-slot="dialog-title" */}
      <DialogDescription>Update your details.</DialogDescription> {/* data-slot="dialog-description" */}
    </DialogHeader>
    <DialogPanel>…</DialogPanel> {/* scrollable body · data-slot="dialog-panel" */}
    <DialogFooter> {/* data-slot="dialog-footer" */}
      <DialogClose render={<Button variant="ghost" />}>Cancel</DialogClose>
      <Button>Save</Button>
    </DialogFooter>
  </DialogPopup>
</Dialog>
```

## Good to know

- `DialogContent` and `DialogOverlay` are aliases of `DialogPopup` and `DialogBackdrop`. Prefer the `Popup` names.
- `DialogPopup` already renders the portal, backdrop and a close (X) button. Don't add them yourself. Hide the X with `showCloseButton={false}`.
- `DialogTrigger` and `DialogClose` are unstyled. Style them with `render={<Button />}`. There is no `asChild`.
- Put long content in `DialogPanel`. It scrolls when the dialog reaches the screen height, while the header and footer stay in place.
- On small screens the dialog becomes a bottom sheet. Turn this off with `bottomStickOnMobile={false}`.
- The state props are `open`, `defaultOpen` and `onOpenChange`. To close after saving, control `open` yourself.
- Always include a `DialogTitle`. It gives the dialog its accessible name.
- For destructive confirmations, use `AlertDialog`. Clicking outside it doesn't close it.

## API Reference

### Dialog

Root provider. Wraps Base UI `Dialog.Root`.

### DialogTrigger

Button that opens the dialog.

### DialogClose

Button that closes the dialog.

### DialogPopup

The modal popup. Also exported as `DialogContent`.

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `showCloseButton` | `boolean` | `true` | Renders an ✕ close button in the top-right corner |
| `bottomStickOnMobile` | `boolean` | `true` | On mobile the popup slides up from the bottom |
| `closeProps` | `DialogPrimitive.Close.Props` | — | Props forwarded to the built-in close button |
| `portalProps` | `Portal.Props` | — | Props forwarded to the portal container |

### DialogHeader

Flex column container for `DialogTitle` and `DialogDescription`.

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `render` | `React.ReactElement` | — | Renders a different element in place of `<div>` |

### DialogFooter

Action button row at the bottom of the dialog.

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `variant` | `"default" \| "bare"` | `"default"` | `default` adds a border-top and background; `bare` removes them |
| `render` | `React.ReactElement` | — | Renders a different element in place of `<div>` |

### DialogTitle

Accessible title element.

### DialogDescription

Accessible description providing additional context.

### DialogPanel

Scrollable content region. Automatically wrapped in `ScrollArea`.

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `scrollFade` | `boolean` | `true` | Fades edges of the scroll area |
| `render` | `React.ReactElement` | — | Renders a different element in place of `<div>` |

## Examples

### Open from a Menu

Demonstrates triggering a `Dialog` from a `MenuItem`, allowing dialogs to be opened as secondary actions from dropdowns.

<ComponentPreview name="v-dialog-2" />

### Bare Footer

Uses `DialogFooter variant="bare"` to remove the border and background from the footer for a minimal, form-like appearance.

<ComponentPreview name="v-dialog-3" />

### With Scroll

Tall content inside `DialogPanel` becomes automatically scrollable via the built-in `ScrollArea` wrapper.

<ComponentPreview name="v-dialog-4" />

### Nested Dialogs

Opens a second dialog from within an already-open dialog, demonstrating stacked overlay behavior and layered focus trapping.

<ComponentPreview name="v-dialog-5" />

### With Destructive Action

A confirmation dialog with a red destructive button for irreversible actions like permanently deleting content.

<ComponentPreview name="v-dialog-6" />

### Cookie Preferences

A settings dialog with toggle switches for managing cookie consent categories such as analytics and marketing.

<ComponentPreview name="v-dialog-7" />

### Session Expired

A blocking dialog notifying the user their session has timed out, with a single "Sign in again" action button.

<ComponentPreview name="v-dialog-8" />

### Edit Profile

A dialog form for updating a user profile that includes an avatar upload area and editable input fields.

<ComponentPreview name="v-dialog-9" />

### Invite Team Members

A dialog for inviting users by email with a role selector and a scrollable list of pending invites.

<ComponentPreview name="v-dialog-10" />

### Share Document

A share dialog with a copyable link field, a one-click copy button that switches to a checkmark on success, and permission-level selector tiles.

<ComponentPreview name="v-dialog-11" />

### Two-Factor Authentication Setup

A 2FA setup dialog displaying a simulated QR code placeholder and a six-digit code entry — verifying the code locks the input and shows a success state.

<ComponentPreview name="v-dialog-12" />

### Subscription Cancellation

A cancellation dialog prompting the user to choose a reason before confirming — the destructive action stays disabled until a reason is selected.

<ComponentPreview name="v-dialog-13" />

### API Key Generation

Generates a new secret API key with mask/reveal toggle, one-click copy with confirmation feedback, and a regenerate button — all within a secure disclosure flow.

<ComponentPreview name="v-dialog-14" />

### Product Feedback Rating

A star-rating feedback form for multiple product aspects (Ease of use, Performance, Design, Support) with an optional comment textarea — Submit stays disabled until every category is rated.

<ComponentPreview name="v-dialog-15" />
