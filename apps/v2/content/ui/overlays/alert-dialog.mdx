...
```tsx
<AlertDialog> {/* root, owns the open state */}
  <AlertDialogTrigger render={<Button variant="destructive-outline" />}>Delete</AlertDialogTrigger> {/* data-slot="alert-dialog-trigger" */}
  <AlertDialogPopup> {/* portal + backdrop + popup · data-slot="alert-dialog-popup" */}
...
- Use an alert dialog when the user must make a choice, such as confirming a deletion. Clicking outside doesn't close it; the user has to pick a button (or press Escape).
- There is no close (X) button and no `AlertDialogPanel`. Keep the content short: a title, a description and the actions.
- There are no `AlertDialogAction` or `AlertDialogCancel` parts. Use `AlertDialogClose` for both buttons and style them with `render={<Button />}`.
- To run code on confirm, add `onClick` to the confirm `AlertDialogClose`. To keep the dialog open while it runs, control `open` yourself and use a normal `Button`.
- `AlertDialogContent` and `AlertDialogOverlay` are aliases of `AlertDialogPopup` and `AlertDialogBackdrop`.
- On small screens it becomes a bottom sheet. Turn this off with `bottomStickOnMobile={false}`.
