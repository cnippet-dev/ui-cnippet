...
```tsx
<Popover> {/* opens and closes the picker */}
  <PopoverTrigger render={<Button variant="outline" />}> {/* shows the chosen date */}
    <CalendarIcon />
    {date ? format(date, "PPP") : "Pick a date"}
  </PopoverTrigger>
  <PopoverPopup> {/* the floating panel */}
...
- There is no `DatePicker` component to install. It's a pattern: a `Calendar` inside a `Popover`. Install both (`@cnippet/calendar` and `@cnippet/popover`).
- The examples format dates with `date-fns`, which isn't installed with the calendar. Run `npm install date-fns`, or format dates another way.
- The popover doesn't close when a date is picked. To close it, control `open` on `Popover` and set it to `false` in `onSelect`.
- For a date range, use `mode="range"` and store a `DateRange` (`{ from, to }`).
...
| `autoFocus` | `boolean` | — | Auto-focuses the calendar when it mounts inside a popover |
