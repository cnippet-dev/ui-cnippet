---
title: Button
description: A button or a component that looks like a button. Built with Base UI and Tailwind CSS. Copy-paste ready.
links:
...
```tsx
<Button variant="outline" size="sm"> {/* <button type="button"> · data-slot="button" */}
  <PlusIcon /> {/* optional icon, sized automatically */}
  Label
</Button>
```
...
- Built on Base UI, not Radix. To render a link or another component, use the `render` prop: `<Button render={<Link href="/login" />}>Login</Button>`. There is no `asChild`.
- The default `type` is `"button"`, not `"submit"`. Pass `type="submit"` inside forms. When you use `render`, no `type` is set.
- `loading` disables the button, hides the label and shows a spinner in its place, so the width doesn't change.
- For icon-only buttons, use one of the `icon` sizes (`icon-xs` to `icon-xl`) and add an `aria-label`.
- `buttonVariants({ variant, size })` returns the class names, for styling an element that isn't a `Button`.
- Sizes are one step taller below the `sm` breakpoint, for easier tapping on touch screens.
...
|------|------|---------|-------------|
| `variant` | `"default" \| "outline" \| "ghost" \| "destructive" \| "destructive-outline" \| "secondary" \| "link"` | `"default"` | Visual style of the button |
| `size` | `"xs" \| "sm" \| "default" \| "lg" \| "xl" \| "icon" \| "icon-xs" \| "icon-sm" \| "icon-lg" \| "icon-xl"` | `"default"` | Size of the button |
| `loading` | `boolean` | `false` | Shows a spinner and disables interaction while true |
| `render` | `React.ReactElement` | — | Renders a different element (e.g. `<Link>`) in place of `<button>` |
...
<ComponentPreview name="v-button-26" />
...
<ComponentPreview name="v-button-27" />
...
<ComponentPreview name="v-button-28" />
...
<ComponentPreview name="v-button-29" />
...
<ComponentPreview name="v-button-30" />
