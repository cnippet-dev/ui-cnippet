---
title: Toggle
description: A two-state button that can be on or off. Built with Base UI and Tailwind CSS. Copy-paste ready.
...
```tsx
<Toggle aria-label="Bold"> {/* <button> with aria-pressed · data-slot="toggle" */}
  <BoldIcon />
</Toggle>
```
...
- The state props are `pressed`, `defaultPressed` and `onPressedChange`.
- For a set of related toggles, use `ToggleGroup` instead. It adds shared state and arrow-key navigation.
- Icon-only toggles need an `aria-label`.
- Style the pressed state with the `data-pressed` attribute.
- `toggleVariants({ variant, size })` returns the class names, for styling an element that isn't a `Toggle`.

## API Reference

### Toggle

Styled wrapper around Base UI's `Toggle` primitive.
...
<ComponentPreview name="v-toggle-11" />
...
<ComponentPreview name="v-toggle-12" />
...
<ComponentPreview name="v-toggle-13" />
...
<ComponentPreview name="v-toggle-14" />
...
<ComponentPreview name="v-toggle-15" />
