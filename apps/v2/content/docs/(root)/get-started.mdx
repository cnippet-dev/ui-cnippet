...
```bash
npx shadcn@latest add @cnippet/button @cnippet/card @cnippet/dialog
```
...
This installs both the components and our default zinc-based color system. The CSS variables are automatically added to your `globals.css`. See the [Styling](/docs/theming) guide for details.
...
1. Browse the [Components](/explore) page and pick one (e.g., Button).
...
- Browse all [Components](/explore)
- Learn about [Styling](/docs/theming) – custom themes, dark mode, and more
- See [Blocks](https://blocks.cnippet.dev/sections) for full page examples built with cnippet ui

## Need help?

- Open an issue on [GitHub](https://github.com/cnippet-dev/ui-cnippet/issues)
