...
export default function TextMorphSearch() {
  ...
  const placeholder = suggestions[query.length % suggestions.length] ?? suggestions[0];
  ...
  return (
    <div className="flex min-h-50 flex-col items-center justify-center gap-4 px-6">
      <div className="relative w-full max-w-md">
        ...
        <input
          className="h-10 w-full rounded-lg border border-border bg-background pl-9 pr-4 text-sm text-foreground outline-none placeholder:text-transparent focus:ring-2 focus:ring-ring"
        ...
        {!query && (
          <div className="pointer-events-none absolute inset-y-0 left-9 flex items-center">
            <TextMorph className="text-sm text-muted-foreground">
              {placeholder}
            </TextMorph>
          </div>
        )}
